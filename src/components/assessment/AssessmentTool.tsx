 ...
 * Wraps both individual and organizational assessments in tabs.
 */
export function AssessmentTool() {
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="text-center space-y-4 mb-8">
                <h2 className="text-3xl font-black text-slate-900">Itsearviointityökalut</h2>
...
function AssessmentRunner({ categories, mode, labels }: { categories: Category[], mode: 'individual' | 'organizational', labels: any }) {
    ...
    const nextCategory = () => {
        // Fix: Ensure implicit answers (Likert defaults) are recorded before moving on
        ...
        currentQuestions.forEach(q => {
            if (answers[q.id] === undefined) {
                // If it's a slider, the visual default is 3. Record it.
