...
export default function DocumentsPage() {
    ...
    if (!mounted) return <div className="p-10 text-center animate-pulse">Ladataan työkaluja...</div>;
                    ...
                    <div className="print:hidden space-y-6">
                        ...
                        <div className="bg-indigo-900 text-white p-8 rounded-3xl shadow-xl flex flex-col md:flex-row gap-6 justify-between items-center relative overflow-hidden">
                            ...
                            <div className="relative z-10 space-y-2">
                                <h1 className="text-3xl font-bold">Työkalut & Asiakirjat</h1>
                                <p className="text-indigo-200 text-lg max-w-lg">
                                    Hallitse dokumentteja, käytä valmiita viestipohjia ja luo älykäs tilannekuva.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <Tabs defaultValue="report" className="w-full">
                        <TabsList className="grid w-full grid-cols-3 mb-8 h-14 p-1 bg-slate-100 rounded-xl">
                            <TabsTrigger value="report" className="rounded-lg text-sm font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all h-full">
                                <Printer className="w-4 h-4 mr-2" /> Raportti
                            </TabsTrigger>
                            <TabsTrigger value="templates" className="rounded-lg text-sm font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all h-full">
                                <FileCheck className="w-4 h-4 mr-2" /> Viestipohjat
                            </TabsTrigger>
                            <TabsTrigger value="premium" className="rounded-lg text-sm font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all h-full text-indigo-700 bg-indigo-50/50">
                                <Sparkles className="w-4 h-4 mr-2" /> AI-Analyysi (Premium)
                            </TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="report" className="focus-visible:outline-none focus-visible:ring-0">
                            <div className="print:p-0 print:shadow-none bg-white p-8 md:p-12 rounded-xl shadow-sm border border-slate-100 max-w-4xl mx-auto space-y-10">
                                {/* ... (Existing Report Content - Keeping it concise for this tool call, assumed unchanged logic) ... */}
                                {/* Re-pasting standard report structure for completeness */}
                                <div className="border-b border-slate-200 pb-8 flex flex-col md:flex-row justify-between items-start gap-4">
                                    ...
                                    <div className="text-right text-sm text-slate-400 print:hidden">
                                        <Button onClick={handlePrint} variant="outline" size="sm" className="gap-2">
                                            <Printer className="w-4 h-4" /> Tulosta PDF
                                        </Button>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <h3 className="text-xl font-bold text-slate-800 border-b pb-2">Tarkka tapahtumalistaus</h3>
                                    {events.length === 0 ? (
                                ...
                                {filteredTemplates.map(template => (
                                    ...
                                    >
                                        <CardHeader className="pb-4">
                                            ...
                                            <CardTitle className="text-xl font-bold leading-tight group-hover:text-indigo-900 transition-colors break-words">{template.title}</CardTitle>
                                        ...
                                        <CardFooter className="mt-auto pt-0">
                                            <Button variant="ghost" className="w-full justify-between items-center text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 font-bold px-0">
                                                Lue ja käytä <ExternalLink className="w-4 h-4 ml-2" />
                                            </Button>
                                        </CardFooter>
                                    </Card>
                                ))}
                                            ...
                                            <div className="p-6 sm:p-10 space-y-8 overflow-y-auto custom-scrollbar bg-slate-50/30">
                                                ...
                                                <section className="space-y-3">
                                                    <h4 className="text-indigo-900 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                                                        <Mail className="w-4 h-4 text-indigo-500" /> Otsikko / Aihe
                                                    </h4>
                                                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group">
                                                        <code className="text-indigo-700 font-mono text-sm">{selectedTemplate.subject}</code>
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                ...
                                                <section className="space-y-3">
                                                    <h4 className="text-indigo-900 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                                                        <FileText className="w-4 h-4 text-indigo-500" /> Viestin sisältö
                                                    </h4>
                                                    ...
                                                    <p className="text-xs text-slate-400 italic flex items-center gap-2">
                                                        <Info className="w-3 h-3" /> Voit muokata tekstiä maalaamalla ja kopioimalla tai käyttää alla olevaa painiketta.
                                                    </p>
                                                </section>
                                            </div>
                                            ...
                                            <DialogFooter className="p-6 bg-white border-t border-slate-100 flex flex-col sm:flex-row gap-3">
                                                ...
                                                >
                                                    {copiedId === selectedTemplate.id ? (
                                                        <><Check className="w-5 h-5 mr-2" /> Kopioitu leikepöydälle</>
                                                    ) : (
                                                        <><Copy className="w-5 h-5 mr-2" /> Kopioi koko teksti</>
                                                    )}
                                                </Button>
                                            </DialogFooter>
                                        </div>
                                ...
                                {aiStep === 'intro' && (
                                    <div className="relative z-10 max-w-2xl text-center space-y-8 animate-in fade-in zoom-in duration-500">
                                        ...
                                        <div className="space-y-4">
                                            <h2 className="text-4xl font-bold">Älykäs Tilanneanalyysi</h2>
                                            <p className="text-indigo-200 text-lg leading-relaxed">
                                                Turvasiipi AI analysoi merkintäsi ja luo ammattimaisen yhteenvedon tilanteestasi.
                                                Raportti sisältää tapahtumien toistuvuuden analyysin, juridisen viitekehyksen ja konkreettiset toimenpidesuositukset.
                                            </p>
                                        </div>
                                        ...
                                        <div className="grid md:grid-cols-3 gap-4 text-left">
                                            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                                ...
                                                <h3 className="font-bold">Anonymisoitu</h3>
                                                <p className="text-xs text-indigo-200 mt-1">Nimet ja paikat poistetaan automaattisesti ennen analyysiä.</p>
                                            </div>
                                            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                                <BrainCircuit className="w-6 h-6 text-purple-400 mb-2" />
                                                <h3 className="font-bold">GPT-4 Analyysi</h3>
                                                <p className="text-xs text-indigo-200 mt-1">Hyödyntää markkinoiden kehittyneintä kielimallia.</p>
                                            </div>
                                            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                                <FileText className="w-6 h-6 text-blue-400 mb-2" />
                                                <h3 className="font-bold">Valmis PDF</h3>
                                                <p className="text-xs text-indigo-200 mt-1">Saat heti valmiin raportin, jonka voit toimittaa eteenpäin.</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-4">
                                            <Button onClick={handleStartPremiumProcess} size="lg" className="bg-white text-indigo-900 hover:bg-indigo-50 font-bold text-lg h-14 px-8 shadow-lg shadow-indigo-900/50">
                                                Aloita analyysi (4.99 €)
                                            </Button>
                                            <p className="text-xs text-indigo-400 mt-3">Kertamaksu. Ei sitoutumista.</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {aiStep === 'payment' && (
                                    <div className="relative z-10 w-full max-w-md bg-white rounded-2xl text-slate-900 p-8 shadow-2xl animate-in slide-in-from-right duration-500">
                                        <div className="flex justify-between items-center mb-6">
                                            <h3 className="text-xl font-bold">Vahvista tilaus</h3>
                                        ...
                                        <div className="space-y-6">
                                                ...
                                                <div className="flex items-start space-x-3">
                                                    ...
                                                    <div className="grid gap-1.5 leading-none">
                                                        <label htmlFor="consent" className="text-sm font-medium leading-tight cursor-pointer">
                                                            Hyväksyn tietojenkäsittelyn
                                                        </label>
                                                        <p className="text-xs text-slate-500">
                                                            Ymmärrän, että anonymisoitu data lähetetään OpenAI:n rajapintaan analysoitavaksi.
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            >
                                                <CreditCard className="w-5 h-5 mr-2" /> Maksa ja analysoi
                                            </Button>
                                ...
                                {aiStep === 'processing' && (
                                    <div className="relative z-10 text-center space-y-6 animate-in fade-in duration-500">
                                        ...
                                        <div className="space-y-2">
                                            <h3 className="text-2xl font-bold">Analysoidaan aineistoa...</h3>
                                            <p className="text-indigo-200">Tämä kestää noin 10-20 sekuntia.</p>
                                        </div>
                                        <div className="text-sm text-indigo-300/80 font-mono">
                                            Anonymisoidaan nimiä... <br />
                                            Tunnistetaan toistuvuuksia... <br />
                                            Luodaan juridista viitekehystä...
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <div className="relative z-10 w-full max-w-3xl bg-white text-slate-900 rounded-xl shadow-2xl overflow-hidden animate-in scale-95 duration-500">
                                        <div className="bg-emerald-50 p-4 border-b border-emerald-100 flex justify-between items-center">
                                            <div className="flex items-center gap-2 text-emerald-800 font-bold">
                                                <Check className="w-5 h-5" /> Analyysi valmis
                                            </div>
                                        ...
                                        <div className="p-4 bg-slate-50 border-t flex justify-end gap-3">
                                            <Button variant="outline" onClick={() => navigator.clipboard.writeText(aiResult.report || "")}>
                                                <Copy className="w-4 h-4 mr-2" /> Kopioi
                                            </Button>
                                            <Button className="bg-indigo-600 hover:bg-indigo-700">
                                                <Download className="w-4 h-4 mr-2" /> Lataa PDF
                                            </Button>
                                        </div>
                                    </div>
                                )}
